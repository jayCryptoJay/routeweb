...
export default function DirectionsPanel({ stop }: { stop: StopDestination | null }) {
  ...
  const destination = stop
    ? encodeURIComponent(stop.lat != null && stop.lng != null ? `${stop.lat},${stop.lng}` : stop.address)
    : "";
  ...
  if (!stop) {
    return (
      <div className="rounded-3xl border border-white/10 bg-white/[0.035] p-5 text-center">
        ...
        <p className="mt-3 text-sm font-semibold text-slate-400">Select a stop to open navigation</p>
      </div>
    );
  }

  return (
    <section className="overflow-hidden rounded-3xl border border-sky-400/20 bg-sky-400/[0.06]">
      <div className="flex items-start justify-between gap-4 p-5">
        <div>
          <div className="flex items-center gap-2 text-sky-300">
            <Navigation size={17} />
            <p className="text-[10px] font-black uppercase tracking-[0.22em]">Navigation</p>
          </div>
        ...
        <button
          ...
          className="grid min-h-11 min-w-11 place-items-center rounded-xl border border-white/10 bg-white/5 text-slate-300 active:scale-95 disabled:opacity-50"
          aria-label="Refresh route preview"
        >
      ...
      <div className="grid grid-cols-2 gap-3 border-t border-white/10 px-5 py-4">
        <a href={`https://maps.apple.com/?daddr=${destination}&dirflg=d`} className="flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-sky-400 px-3 text-center text-sm font-black text-slate-950 active:scale-[0.98]">
          <Navigation size={17} />Apple Maps
        </a>
        <a href={`https://www.google.com/maps/dir/?api=1&destination=${destination}&travelmode=driving`} className="flex min-h-14 items-center justify-center gap-2 rounded-2xl border border-white/10 bg-[#1F2937] px-3 text-center text-sm font-black text-white active:scale-[0.98]">
          <Navigation size={17} />Google Maps
        </a>
      </div>
      <p className="px-5 pb-4 text-center text-[11px] leading-relaxed text-slate-500">Use your navigation app for turn-by-turn guidance. Review this preview only while parked or safely stopped.</p>

      <div className="border-t border-white/10 px-5 py-4">
        {directions.isLoading ? (
          <div className="flex items-center gap-3 text-sm text-slate-400"><div className="h-4 w-4 animate-spin rounded-full border-2 border-sky-300 border-t-transparent" />Calculating route preview…</div>
        ) : directions.error ? (
          <div className="rounded-2xl bg-black/20 p-4 text-sm text-amber-300">The in-app preview is temporarily unavailable. Apple Maps and Google Maps above are still ready to navigate this stop.</div>
        ) : directions.data ? (
          <>
            <div className="mb-4">
      ...
      >
        <LocateFixed size={15} />Use my current location for preview
      </button>
    </section>
  );
}
